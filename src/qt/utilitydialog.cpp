    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("Credits") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About Credits"));
        ...
        std::string strUsage = HelpMessage(HMM_CREDITS_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", CreditsGUI::DEFAULT_UIPLATFORM));
        }
...
All the Credits in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 CRDS, 0.1 CRDS, 1 CRDS, 10 CRDS -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("Credits is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

void ShutdownWindow::showShutdownWindow(CreditsGUI *window)
{
    if (!window)
        return;
